...
function fmtAxisValue(v, unit) {
  switch (unit) {
    ...
    case 'ratio': return (+v).toFixed(2);
    case 'level': return Math.round(v / 1000).toLocaleString() + 'k';
    case 'count': return Math.round(v).toLocaleString();
    case 'pct':
    default:      return (v * 100).toFixed(1) + '%';
  }
}

// Canonical chart renderer — real y-axis tick labels, several evenly-spaced x-axis
// labels, multi-series support (for the 8-country interest rate overlays). Used by
// dashboard.js's own window charts, index.html's hub chart, and interest-rate.html.
//   canvas   - the <canvas> element (its width/height attributes set the CSS size)
//   series   - array of { label, color, points: [{date, value}] }; single series is fine too
//   unit     - 'pct' | 'index' | 'level' | 'count' | 'ratio'
function drawChart(canvas, series, unit) {
  ...
  series = series.filter(function (s) { return s.points && s.points.length >= 2; });
  if (!series.length) {
    ctx.fillStyle = '#8a8f98';
    ctx.font = '12px -apple-system, sans-serif';
    ctx.fillText('Not enough history.', 14, 28);
    return;
  }

  const allVals = series.reduce(function (acc, s) { return acc.concat(s.points.map(function (p) { return p.value; })); }, []);
  ...
  const left = 58, right = cssW - 16, top = 14 + legendH, bottom = cssH - 28;
  const longest = series.reduce(function (a, b) { return a.points.length >= b.points.length ? a : b; });
  const n = longest.points.length;
  const x = function (i) { return left + (right - left) * (i / (n - 1)); };
  ...
  for (let i = 0; i <= yTicks; i++) {
    ...
    ctx.fillText(fmtAxisValue(v, unit), left - 8, gy + 3);
  }

  // X-axis tick labels — evenly spaced across the axis, not just first/last
  const xTicks = Math.min(6, n);
  ctx.textAlign = 'center';
  for (let i = 0; i < xTicks; i++) {
    const idx = Math.round((n - 1) * (i / (xTicks - 1)));
    ctx.fillText(longest.points[idx].date, x(idx), cssH - 8);
  }
  ...
  if (series.length > 1) {
    ...
    series.forEach(function (s) {
      ...
      ctx.beginPath(); ctx.arc(lx + 4, top - legendH + 8, 4, 0, Math.PI * 2); ctx.fill();
      ctx.fillStyle = '#c4c8ce';
      ctx.textAlign = 'left';
      ctx.fillText(s.label, lx + 12, top - legendH + 12);
      lx += ctx.measureText(s.label).width + 34;
    });
  }

  // Lines (+ area fill for single-series charts)
  series.forEach(function (s) {
    if (series.length === 1) {
      ...
      ctx.lineTo(x(s.points.length - 1), bottom);
    ...
    const lastX = x(s.points.length - 1), lastY = y(lastP.value);
    ctx.fillStyle = s.color;
    ...
    if (series.length === 1) {
      ...
      ctx.textAlign = 'right';
      ctx.fillText(fmtMetric(lastP.value, unit), lastX, lastY - 10);
    }
  });
}

function drawLineChart(canvas, points, unit) {
  drawChart(canvas, [{ label: '', color: '#5884ff', points: points }], unit);
}
...
function selectMetric(data, key) {
  ...
  const summaryRow = document.getElementById('trend-summary-row');
  if (summaryRow) {
    if (m.unit === 'text') {
      summaryRow.innerHTML = '<td>' + m.label + '</td><td colspan="4">' + (m.current || '—') + '</td>';
    } else {
      summaryRow.innerHTML =
        '<td>' + m.label + '</td>' +
        '<td class="cur">' + fmtMetric(m.current, m.unit) + '<div class="cell-date">' + m.currentDate + '</div></td>' +
